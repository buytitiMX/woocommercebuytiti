  ...
  const fetchProductsByCategory = async () => {
    try {
      ...
      for (let page = 1; page <= 5; page++) {
        const response = await fetch(`http://localhost/wordpress/wp-json/wc/store/products?category=${selectedCategory}&per_page=100&page=${page}`);
        if (!response.ok) {
      ...
      const productsWithIsNew = await Promise.all(categoryProducts.map(async product => {
        try {
  ...
  const fetchRecentProducts = async () => {
    try {
      ...
      for (let page = 1; page <= 1; page++) {
        const response = await fetch(`http://localhost/wordpress/wp-json/wc/store/products?per_page=10&page=${page}`);
        if (!response.ok) {
          throw new Error(`Error en la solicitud: ${response.statusText}`);
        }
        const data = await response.json();
      ...
      const productsWithIsNew = await Promise.all(recentProducts.map(async product => {
        try {
          const productResponse = await fetch(`http://localhost/wordpress/wp-json/wp/v2/product/${product.id}`);
          if (!productResponse.ok) {
            throw new Error(`Error en la solicitud de producto: ${productResponse.statusText}`);
      ...
      console.error("Error fetching recent WooCommerce products:", error.message);
      setLoading(false);
    }
  };
